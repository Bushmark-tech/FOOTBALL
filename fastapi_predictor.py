...
async def predict(request: PredictionRequest):
    ...
    try:
        ...
        # Get team form data (last 5 matches) - same as result view
        try:
            # Try both datasets to find form data
            form_home = None
            form_away = None
            
            # Try dataset 1 first
            data1 = FOOTBALL_DATA_CACHE.get('data1')
            if data1 is None:
                data1 = analytics.load_football_data('data1', use_cache=True)
            
            if data1 is not None and not (hasattr(data1, 'empty') and data1.empty):
                try:
                    form_home = get_team_recent_form_original(request.home_team, data1, version="v1")
                    form_away = get_team_recent_form_original(request.away_team, data1, version="v1")
                except:
                    pass
            
            # If no form data or hash-based forms, try dataset 2
            if (not form_home or form_home == 'DDDDD' or not form_away or form_away == 'DDDDD'):
                data2 = FOOTBALL_DATA_CACHE.get('data2')
                if data2 is None:
                    data2 = analytics.load_football_data('data2', use_cache=True)
                
                if data2 is not None and not (hasattr(data2, 'empty') and data2.empty):
                    try:
                        if not form_home or form_home == 'DDDDD':
                            form_home_d2 = get_team_recent_form_original(request.home_team, data2, version="v2")
                            if form_home_d2 and form_home_d2 != 'DDDDD':
                                form_home = form_home_d2
                        if not form_away or form_away == 'DDDDD':
                            form_away_d2 = get_team_recent_form_original(request.away_team, data2, version="v2")
                            if form_away_d2 and form_away_d2 != 'DDDDD':
                                form_away = form_away_d2
                    except:
                        pass
            
            # Ensure form strings are valid (5 characters, pad if needed)
            if form_home and form_home != 'DDDDD':
                if len(form_home) > 5:
                    form_home = form_home[:5]
                elif len(form_home) < 5:
                    form_home = form_home + ("D" * (5 - len(form_home)))
            else:
                form_home = None
                
            if form_away and form_away != 'DDDDD':
                if len(form_away) > 5:
                    form_away = form_away[:5]
                elif len(form_away) < 5:
                    form_away = form_away + ("D" * (5 - len(form_away)))
            else:
                form_away = None
                
            print(f"[FORM DATA] {request.home_team}: {form_home}, {request.away_team}: {form_away}")
        except Exception as e:
            print(f"[WARNING] Could not retrieve form data: {e}")
            import traceback
            traceback.print_exc()
            form_home = None
            form_away = None
