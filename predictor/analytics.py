...
# ============================================================================
# Team Name Normalization - CRITICAL FIX for "Man City" vs "Manchester City"
# ============================================================================
...
def find_team_in_data(team_name, data, column_name):
    ...
    for team in unique_teams:
        ...
        if team_lower in team_str or team_str in team_lower:
            logger.info(f"Matched '{team_name}' to '{team}' (partial) in {column_name}")
            return team
    
    # Strategy 5: Normalized partial match
    for team in unique_teams:
        normalized_team_lower = normalize_team_name(str(team)).lower().strip()
        if normalized_input_lower in normalized_team_lower or normalized_team_lower in normalized_input_lower:
            logger.info(f"Matched '{team_name}' to '{team}' (normalized partial) in {column_name}")
            return team
    
    logger.warning(f"Could not find '{team_name}' in {column_name} using any matching strategy")
    return None
...
def calculate_probabilities_model2(home, away, data, version="v2"):
    ...
    if data is None:
        logger.warning(f"No data provided for probability calculation: {home} vs {away}")
        return None
    ...
    if data_empty:
        logger.warning(f"Empty data provided for probability calculation: {home} vs {away}")
        return None
    
    # Auto-detect version from data columns
    if not hasattr(data, 'columns'):
        logger.warning(f"Data does not have columns attribute: {home} vs {away}")
        return None
    ...
    try:
        # CRITICAL FIX: Use team name matching to handle "Man City" vs "Manchester City"
        home = str(home).strip()
        away = str(away).strip()
        ...
        # If teams not found, return None to trigger fallback
        if home_matched is None or away_matched is None:
            logger.warning(f"Teams not found in dataset: {home} (matched: {home_matched}) vs {away} (matched: {away_matched})")
            return None
        
        # Use matched names for H2H lookup
        logger.info(f"Using matched names: '{home}' -> '{home_matched}', '{away}' -> '{away_matched}'")
        
        # Direction 1: home team is HOME, away team is AWAY (most relevant)
        mask1 = (data[home_col].astype(str).str.strip() == home_matched) & (data[away_col].astype(str).str.strip() == away_matched)
        h2h_dir1 = data[mask1]
        
        # Direction 2: home team is AWAY, away team is HOME (flip results for context)
        mask2 = (data[home_col].astype(str).str.strip() == away_matched) & (data[away_col].astype(str).str.strip() == home_matched)
        h2h_dir2 = data[mask2]
        
        # Check if we have ANY H2H data from either direction
        total_h2h_matches = len(h2h_dir1) + len(h2h_dir2)
        
        # FIXED: Only return None if we have NO H2H data at all
        # Even 1-2 matches can provide useful historical context
        if total_h2h_matches == 0:
            logger.info(f"NO H2H data found for {home} vs {away} - returning None for fallback")
            return None
        
        # Log that we found H2H data
        logger.info(f"Found {total_h2h_matches} H2H matches for {home} vs {away} ({len(h2h_dir1)} direct, {len(h2h_dir2)} reverse)")
        
        # Initialize counts for both directions
        counts1 = None
        counts2 = None
        
        # Direction 1: home is HOME, away is AWAY (direct match)
        # H = home wins, D = draw, A = away wins
        if len(h2h_dir1) > 0:
            counts1 = h2h_dir1[result_col].value_counts()
        
        # Direction 2: home is AWAY, away is HOME (flip perspective)
        if len(h2h_dir2) > 0:
            counts2 = h2h_dir2[result_col].value_counts()
        
        # Calculate probabilities with proper weighting for home advantage
        # Direction 1 (home team was home) is most relevant
        # Direction 2 (home team was away) provides supplementary data
        total_dir1 = len(h2h_dir1)
        total_dir2 = len(h2h_dir2)
        total = total_dir1 + total_dir2
        
        # This should never happen since we checked above, but keep for safety
        if total == 0:
            logger.warning(f"Unexpected: total H2H matches is 0 for {home} vs {away} - returning None")
            return None
        
        # If we have matches in both directions, weight them to reflect home advantage
        # Home matches (dir1) get full weight (1.0), away matches (dir2) get reduced weight (0.6)
        # This ensures reversed fixtures get different probabilities
        if total_dir1 > 0 and total_dir2 > 0:
            # Calculate weighted probabilities
            # For direction 1: use actual counts
            dir1_home = counts1.get('H', 0) if counts1 is not None else 0
            dir1_draw = counts1.get('D', 0) if counts1 is not None else 0
            dir1_away = counts1.get('A', 0) if counts1 is not None else 0
            
            # For direction 2: use flipped counts with reduced weight
            dir2_home = counts2.get('A', 0) if counts2 is not None else 0  # A when they were away = our home win
            dir2_draw = counts2.get('D', 0) if counts2 is not None else 0
            dir2_away = counts2.get('H', 0) if counts2 is not None else 0  # H when they were home = our away win
            
            # Weight: dir1 gets 1.0, dir2 gets 0.6 (home advantage factor)
            weighted_home = (dir1_home * 1.0 + dir2_home * 0.6)
            weighted_draw = (dir1_draw * 1.0 + dir2_draw * 0.6)
            weighted_away = (dir1_away * 1.0 + dir2_away * 0.6)
            weighted_total = (total_dir1 * 1.0 + total_dir2 * 0.6)
        
            logger.info(f"H2H {home} vs {away}: {total} matches ({total_dir1} home @1.0, {total_dir2} away @0.6) - Weighted H:{weighted_home:.1f}, D:{weighted_draw:.1f}, A:{weighted_away:.1f}")
            
            return {
                "Home Team Win": (weighted_home / weighted_total * 100) if weighted_total > 0 else 33.3,
                "Draw": (weighted_draw / weighted_total * 100) if weighted_total > 0 else 33.3,
                "Away Team Win": (weighted_away / weighted_total * 100) if weighted_total > 0 else 33.3,
            }
        elif total_dir1 > 0:
            # Only direction 1 available (home team was home)
            counts1 = h2h_dir1[result_col].value_counts()
            home_wins = counts1.get('H', 0)
            draws = counts1.get('D', 0)
            away_wins = counts1.get('A', 0)
            
            logger.info(f"H2H {home} vs {away}: {total_dir1} matches (home only) - H:{home_wins}, D:{draws}, A:{away_wins}")
            
            return {
                "Home Team Win": (home_wins / total_dir1 * 100) if total_dir1 > 0 else 33.3,
                "Draw": (draws / total_dir1 * 100) if total_dir1 > 0 else 33.3,
                "Away Team Win": (away_wins / total_dir1 * 100) if total_dir1 > 0 else 33.3,
            }
        else:
            # Only direction 2 available (home team was away, flip results)
            counts2 = h2h_dir2[result_col].value_counts()
            # Flip: H becomes away win, A becomes home win
            home_wins = counts2.get('A', 0)  # They were away, A = their loss = our home win
            draws = counts2.get('D', 0)
            away_wins = counts2.get('H', 0)  # They were home, H = their win = our away win
            
            logger.info(f"H2H {home} vs {away}: {total_dir2} matches (away only, flipped) - H:{home_wins}, D:{draws}, A:{away_wins}")
            
            return {
                "Home Team Win": (home_wins / total_dir2 * 100) if total_dir2 > 0 else 33.3,
                "Draw": (draws / total_dir2 * 100) if total_dir2 > 0 else 33.3,
                "Away Team Win": (away_wins / total_dir2 * 100) if total_dir2 > 0 else 33.3,
        }
    except Exception as e:
...
def calculate_probabilities_original(home, away, data, version="v1"):
    ...
    try:
        ...
        if home_matched is None or away_matched is None:
            logger.info(f"Teams not found in dataset: {home_str} (matched: {home_matched}) vs {away_str} (matched: {away_matched}) - using form-based fallback")
            enhanced_features = get_enhanced_features(home, away)
            home_strength = enhanced_features['home_strength']
            away_strength = enhanced_features['away_strength']
            strength_diff = home_strength - away_strength
        ...
        # Use matched names for H2H lookup
        logger.info(f"Using matched names: '{home_str}' -> '{home_matched}', '{away_str}' -> '{away_matched}'")
        
        # Get matches where home is home team and away is away team
        mask1 = data[home_col].astype(str).str.strip() == home_matched
        h2h_home = data[mask1]
        if len(h2h_home) > 0:
            mask2 = h2h_home[away_col].astype(str).str.strip() == away_matched
        ...
        mask3 = data[home_col].astype(str).str.strip() == away_matched
        h2h_away = data[mask3]
        if len(h2h_away) > 0:
            mask4 = h2h_away[away_col].astype(str).str.strip() == home_matched
...
def get_team_result_in_match(row, team_name, version="v1"):
    ...
    try:
        ...
        if pd.notna(row[home_col]) and str(row[home_col]).strip() == str(team_name).strip():
            # Support both Dataset 1 (FTHG) and Dataset 2 (HG)
            goals_scored = row.get('FTHG', row.get('HG', 0))
            goals_conceded = row.get('FTAG', row.get('AG', 0))
            ftr = row[result_col]
        ...
        # Check if team is away team
            ...
            # Support both Dataset 1 (FTAG) and Dataset 2 (AG)
            goals_scored = row.get('FTAG', row.get('AG', 0))
            goals_conceded = row.get('FTHG', row.get('HG', 0))
            ftr = row[result_col]
...
def load_team_mapping():
    ...
    try:
        ...
        paths_to_try = [
            os.path.join(os.path.dirname(__file__), '..', 'data', 'team_mapping.csv'),
            os.path.join(os.path.dirname(__file__), '..', '..', 'data', 'team_mapping.csv'),
...
def preprocess_for_models(home_team, away_team, model, data=None):
    ...
    try:
        ...
        # Check if model relies on Team IDs (Model 2)
        use_id_mapping = False
        if hasattr(model, 'feature_names_in_'):
            # Detect Model 2 features (HomeTeam, AwayTeam as raw column names, not one-hot)
            if 'HomeTeam' in model.feature_names_in_ and 'AwayTeam' in model.feature_names_in_:
                use_id_mapping = True
                logger.info("Model detected as ID-based (Model 2)")
        ...
        # MOVED TO AFTER COLUMN DETECTION TO USE CORRECT VERSION
        home_form = None 
        away_form = None
        ...
        # Calculate goal differences (later after form found)
        home_goal_diff = 0.0
        away_goal_diff = 0.0
        form_goal_diff = 0.0
        ...
        if 'HomeTeam' in data.columns and 'AwayTeam' in data.columns:
            ...
            away_col = 'AwayTeam'
            dataset_version = 'v1'
        elif 'Home' in data.columns and 'Away' in data.columns:
            # Dataset 2 format
            home_col = 'Home'
            away_col = 'Away'
            dataset_version = 'v2'
        ...
        mask_home_home = data[home_col].astype(str).str.strip() == home_str
        mask_home_away = data[away_col].astype(str).str.strip() == home_str
        ...
        mask_away_home = data[home_col].astype(str).str.strip() == away_str
        mask_away_away = data[away_col].astype(str).str.strip() == away_str
        ...
        # OPTIMIZED: Use already computed masks or filter efficiently
        home_team_home_matches = data[mask_home_home] if 'mask_home_home' in locals() else data[data[home_col].astype(str).str.strip() == home_str]
        away_team_away_matches = data[mask_away_away] if 'mask_away_away' in locals() else data[data[away_col].astype(str).str.strip() == away_str]
        ...
        if use_id_mapping:
            # Model 2 specific features (IDs + Advanced Stats)
            team_mapping = load_team_mapping()
            
            # Map IDs
            home_id = team_mapping.get(home_team.strip(), -1)
            away_id = team_mapping.get(away_team.strip(), -1)
            
            features_dict['HomeTeam'] = home_id
            features_dict['AwayTeam'] = away_id
            
            if home_id == -1:
                logger.warning(f"Team ID not found for home team: '{home_team}' - Model 2 features may be inaccurate")
            if away_id == -1:
                logger.warning(f"Team ID not found for away team: '{away_team}' - Model 2 features may be inaccurate")
            
            # Helper to safely get scalar value
            def safe_get(key, default=1.0):
                return float(features_dict.get(key, default))

            # Advanced Stats Calculation for Model 2
            # 1. Attack/Defense Strength (Goals / League Avg) - Approx league avg ~1.5
            features_dict['home_attack_strength'] = safe_get('home_goals_scored', 0) / 1.5
            features_dict['home_defense_strength'] = safe_get('home_goals_conceded', 0) / 1.5
            features_dict['away_attack_strength'] = safe_get('away_goals_scored', 0) / 1.5
            # Defense strength usually concession rate
            
            # 2. Odds Analysis (if available)
            b365h = safe_get('B365H', 2.5)
            b365a = safe_get('B365A', 2.5)
            b365d = safe_get('B365D', 3.2)
            
            features_dict['home_away_odds_diff'] = b365h - b365a
            features_dict['favorite_odds'] = min(b365h, b365a)
            
            # 3. Win Rates (Approx from recent form wins / 5 matches)
            features_dict['home_favorite_win_rate'] = (home_wins / 5.0) if home_wins <= 5 else 0.5
            features_dict['away_favorite_win_rate'] = (away_wins / 5.0) if away_wins <= 5 else 0.5
            
            # 4. Other stats
            features_dict['home_corner_diff'] = safe_get('HC', 5.0) - safe_get('AC', 4.0)
            features_dict['home_conceded_at_home'] = safe_get('home_goals_conceded', 1.0) # Approximation
            features_dict['away_goals_conceded'] = safe_get('away_goals_conceded', 1.0)
            features_dict['away_points'] = away_points
            
            # Clean Sheets (Approx based on conceded)
            # If avg conceded < 0.5, likely had clean sheets. Just an estimate.
            features_dict['home_clean_sheets'] = 1 if safe_get('home_goals_conceded', 1.0) < 0.8 else 0
        ...
        if not hasattr(preprocess_for_models, '_module_teams_cache') or preprocess_for_models._module_teams_cache.get('hash') != data_hash:
            ...
            home_teams = data[home_col].dropna().astype(str).unique()
            away_teams = data[away_col].dropna().astype(str).unique()
        ...
        # OPTIMIZATION: Skip one-hot encoding if Model 2 (ID-based) and OHE features not requested
        skip_ohe = False
        if use_id_mapping and hasattr(model, 'feature_names_in_'):
            # Check if any feature starts with HomeTeam_ or AwayTeam_
            has_ohe_features = any(feat.startswith('HomeTeam_') or feat.startswith('AwayTeam_') for feat in model.feature_names_in_)
            if not has_ohe_features:
                skip_ohe = True
                # logger.debug("Skipping one-hot encoding for Model 2 (ID-based)")
...
def advanced_predict_match(home_team, away_team, model1, model2):
    ...
    try:
        ...
        if probs:
            logger.info(f"advanced_predict_match probabilities for {home_team} vs {away_team}: Home={probs.get('Home Team Win', 0):.1f}%, Draw={probs.get('Draw', 0):.1f}%, Away={probs.get('Away Team Win', 0):.1f}%")
        ...
        # This is used for confidence calculation, but MODEL prediction takes priority for final outcome
        if prob_dict:
            prediction_from_probs = max(prob_dict, key=prob_dict.get)
            confidence = float(prob_dict[prediction_from_probs])
            logger.info(f"  - Prediction from probabilities (after form correction): {prediction_from_probs} (0=Away, 1=Draw, 2=Home), confidence: {confidence:.3f}")
            
            # Note: We'll use original_model_prediction for final outcome, not prediction_from_probs
        else:
            # Fallback: try to use pred_label directly
            if pred_label:
                if str(pred_label).upper() in ['A', 'AWAY', '0']:
                    prediction_from_probs = 0
                elif str(pred_label).upper() in ['D', 'DRAW', '1']:
                    prediction_from_probs = 1
                elif str(pred_label).upper() in ['H', 'HOME', '2']:
                    prediction_from_probs = 2
                else:
                    prediction_from_probs = 1  # Default to Draw
            else:
                prediction_from_probs = 1  # Default to Draw
            confidence = 0.34
        ...
        # IMPORTANT: Save original model prediction - Model prediction should always come first
        # pred was set earlier from model.predict() and represents the model's prediction considering current form
        original_model_prediction = pred  # This is the model's prediction considering current form
        ...
        if probs:
            ...
            if max(hist_probs_list) - min(hist_probs_list) > 10:  # More than 10% difference
                use_historical = True
                logger.info("Historical probabilities show clear preference, blending with model probabilities")
        
        if use_historical and probs:
            # Blend historical probabilities with model probabilities instead of replacing completely
            # This ensures current form is still considered even when historical data shows strong trends
            ...
            # Apply minimum threshold to avoid 0% or 100% probabilities
            # Minimum 10% for each outcome, maximum 75% for any single outcome
            # This prevents unrealistic predictions like 100% draw or 0% for any outcome
            MIN_PROB = 0.10  # 10% minimum for each outcome
            MAX_PROB = 0.75  # 75% maximum for any single outcome
            
            # Apply thresholds while preserving relative differences
            hist_prob_dict = {}
            for key in hist_prob_dict_raw:
                raw_val = hist_prob_dict_raw[key]
                # Clamp to min/max
                clamped_val = max(MIN_PROB, min(MAX_PROB, raw_val))
                hist_prob_dict[key] = clamped_val
            
            # Normalize after applying thresholds to ensure they sum to 1.0
            total_hist = sum(hist_prob_dict.values())
            if total_hist > 0:
                hist_prob_dict = {k: v/total_hist for k, v in hist_prob_dict.items()}
            
            # Blend with model probabilities, but adjust weight based on how extreme historical data is
            # If historical data is very extreme (like 100% draw), give more weight to model (current form)
            max_hist_prob = max(hist_prob_dict.values())
            min_hist_prob = min(hist_prob_dict.values())
            extreme_difference = max_hist_prob - min_hist_prob
            
            # Adjust blend weight: more extreme historical data = more weight to model
            # If difference > 0.6 (very extreme), use 40% historical, 60% model
            # If difference < 0.3 (balanced), use 70% historical, 30% model
            if extreme_difference > 0.6:
                blend_weight = 0.4  # 40% historical, 60% model (historical data is too extreme)
            elif extreme_difference > 0.4:
                blend_weight = 0.5  # 50% historical, 50% model (moderate extreme)
            else:
                blend_weight = 0.6  # 60% historical, 40% model (normal case)
            
            logger.info(f"  - Historical data extremeness: {extreme_difference:.2f}, using blend weight: {blend_weight:.1f} (historical), {1-blend_weight:.1f} (model)")
            
            blended_prob_dict = {}
            for key in [0, 1, 2]:
                historical_val = hist_prob_dict[key]
                model_val = prob_dict.get(key, 0.33)  # Default to uniform if model prob not available
                blended_prob_dict[key] = (blend_weight * historical_val) + ((1 - blend_weight) * model_val)
            
            # Normalize blended probabilities
            total_blended = sum(blended_prob_dict.values())
            if total_blended > 0:
                blended_prob_dict = {k: v/total_blended for k, v in blended_prob_dict.items()}
            
            # Update prob_dict with blended probabilities FOR DISPLAY ONLY
            prob_dict = blended_prob_dict
            
            # PRIORITY: Use MODEL prediction AFTER form corrections as the primary decision maker
            # Form corrections adjust the model based on current team form (e.g., Aston Villa's strong form)
            # prediction_from_probs is the prediction after form-based corrections, which considers current form
            # This is what the model predicts after adjusting for current team strength and form
            model_prediction = prediction_from_probs  # Use prediction after form corrections (model's adjusted prediction)
            
            # Check if probabilities are very close - if so, use double chance instead
            prob_home = prob_dict.get(2, 0)
            prob_draw = prob_dict.get(1, 0)
            prob_away = prob_dict.get(0, 0)
            
            # Sort probabilities to find top two
            prob_list = [(prob_home, 2, "Home"), (prob_draw, 1, "Draw"), (prob_away, 0, "Away")]
            prob_list.sort(reverse=True, key=lambda x: x[0])
            top_prob = prob_list[0][0]
            second_prob = prob_list[1][0]
            top_outcome = prob_list[0][2]
            second_outcome = prob_list[1][2]
            
            # If top two probabilities are within 5% of each other, use double chance
            prob_difference = top_prob - second_prob
            if prob_difference < 0.05:  # Within 5% - probabilities are almost the same
                # Create double chance prediction
                if top_outcome == "Home" and second_outcome == "Draw":
                    outcome = "1X"  # Home or Draw
                    final = "Home Team Win or Draw"
                elif top_outcome == "Home" and second_outcome == "Away":
                    outcome = "12"  # Home or Away
                    final = "Home Team Win or Away Team Win"
                elif top_outcome == "Draw" and second_outcome == "Home":
                    outcome = "1X"  # Home or Draw
                    final = "Home Team Win or Draw"
                elif top_outcome == "Draw" and second_outcome == "Away":
                    outcome = "X2"  # Draw or Away
                    final = "Draw or Away Team Win"
                elif top_outcome == "Away" and second_outcome == "Draw":
                    outcome = "X2"  # Draw or Away
                    final = "Draw or Away Team Win"
                elif top_outcome == "Away" and second_outcome == "Home":
                    outcome = "12"  # Home or Away
                    final = "Home Team Win or Away Team Win"
                else:
                ...
                # Use the higher of the two probabilities as confidence
                prediction = prob_list[0][1]  # Use top outcome code for prediction
                confidence = top_prob
            ...
            logger.info(f"  - Historical probabilities (clamped): Away={hist_prob_dict[0]:.1%}, Draw={hist_prob_dict[1]:.1%}, Home={hist_prob_dict[2]:.1%}")
            logger.info(f"  - Blended probabilities ({blend_weight:.0%} historical, {1-blend_weight:.0%} model): Away={prob_dict[0]:.3f}, Draw={prob_dict[1]:.3f}, Home={prob_dict[2]:.3f}")
            logger.info(f"  - MODEL prediction (PRIORITY): {model_prediction} (0=Away, 1=Draw, 2=Home)")
            logger.info(f"  - Final prediction: {prediction} (0=Away, 1=Draw, 2=Home), outcome: {outcome}, confidence: {confidence:.3f}")
            
            # Update probs to use blended probabilities for display (convert back to percentage format)
            probs = {
                "Home Team Win": prob_dict[2] * 100,
                "Draw": prob_dict[1] * 100,
                "Away Team Win": prob_dict[0] * 100
            }
            
            logger.info(f"  - Final outcome: {final}")
        elif probs:
...
class ProfessionalFootballAnalytics:
    ...
    def get_team_form(self, team_name, last_matches=10):
        ...
        try:
            ...
            form_string = None
            if not data_empty:
                # Try to get actual form from data using Model2 function
                try:
                    form_string = get_team_recent_form_model2(team_name, data, version="v2")
                except:
            ...
            if not form_string or len(form_string) < 5 or form_string == "-----":
                import hashlib
                team_hash = int(hashlib.md5(str(team_name).strip().encode()).hexdigest()[:8], 16)
                form_chars = []
                for i in range(5):
            ...
            # Convert form string to list for recent_form
            recent_form = list(form_string[:last_matches]) if form_string else ['W', 'D', 'W', 'L', 'W']
            
            # Generate realistic metrics based on form
            ...
            form_data = {
                ...
                'goals_scored': np.random.randint(8, 25, min(last_matches, len(recent_form))),
                'goals_conceded': np.random.randint(5, 20, min(last_matches, len(recent_form))),
                'possession_avg': np.random.uniform(45, 65, min(last_matches, len(recent_form))),
                'shots_on_target': np.random.randint(3, 8, min(last_matches, len(recent_form))),
                'clean_sheets': max(0, wins - losses),
                'points': points
            }
            return form_data
        except Exception as e:
            logger.error(f"Error getting team form for {team_name}: {e}")
            # Fallback to hash-based form
            try:
                import hashlib
                team_hash = int(hashlib.md5(str(team_name).strip().encode()).hexdigest()[:8], 16)
                form_chars = []
                ...
                form_string = "".join(form_chars)
                return {
                    'recent_form': list(form_string),
                    'goals_scored': np.random.randint(8, 25, 5),
                    'goals_conceded': np.random.randint(5, 20, 5),
                    'possession_avg': np.random.uniform(45, 65, 5),
                    'shots_on_target': np.random.randint(3, 8, 5),
                    'clean_sheets': form_string.count('W'),
                    'points': form_string.count('W') * 3 + form_string.count('D')
                }
            except:
                return None
    ...
    def calculate_team_strength(self, team_name, home_away='home'):
        ...
        try:
            ...
            form_string = None
            if not data_empty:
                # Use actual form from data - auto-detect version from data
                if 'Home' in data.columns and 'Away' in data.columns:
                    actual_version = "v2"
                elif 'HomeTeam' in data.columns and 'AwayTeam' in data.columns:
                    actual_version = "v1"
                    
                form_string = get_team_recent_form_original(team_name, data, version=actual_version)
