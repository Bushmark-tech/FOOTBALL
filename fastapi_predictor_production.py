...
def smart_prediction_logic(model_prediction: str, prob_home: float, prob_draw: float, prob_away: float) -> tuple:
    ...
    if model_prediction != max_prob_outcome and max_prob < 0.45:
        ...
        if "Home" in top_two and "Draw" in top_two:
            ...
            final_pred = "12"
            reasoning = f"Uncertainty between Home ({prob_home*100:.1f}%) and Away ({prob_away*100:.1f}%)"
    ...
    if max_prob > 0.50 and model_prediction != max_prob_outcome:
        ...
        reasoning = f"Historical data strongly suggests {max_prob_outcome} ({max_prob*100:.1f}%), overriding model {model_prediction}"
...
class PredictionResponse(BaseModel):
    ...
    cached: bool = False
    prediction_type: Optional[str] = "Single"
...
async def predict(
    ...
    try:
        ...
        # Extract probabilities with flexible handling
        ...
        if historical_probs and isinstance(historical_probs, dict) and len(historical_probs) > 0:
            # Historical probs are in percentage (0-100)
            prob_dict = {
            ...
            # Fallback to integer/string keyed probabilities
            probs = result.get('probabilities', {})
            prob_dict = {
                "Home": float(probs.get(2, probs.get("Home", 0.33))),
                "Draw": float(probs.get(1, probs.get("Draw", 0.33))),
                "Away": float(probs.get(0, probs.get("Away", 0.33)))
            }
